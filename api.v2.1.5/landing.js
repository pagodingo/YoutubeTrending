module.exports = {
  landing: function (data) {
    ...
    for (var i = 0; i < data.length; ++i){
      ...
      csv += JSON.parse(data[i]).Thumbnail + ','
    }
    let page = `
<!DOCTYPE html>
<html lang="en">
...
<a id="downloadAnchor" style="display:none"></a>
  ...
  <p style="color: lightgrey; margin-bottom: 40px;"><u>https://</u> &nbsp;&nbsp; + &nbsp;&nbsp; <a  style="color: lightgrey" href="https://getyoutubetrending.netlify.app" target="_blank">getyoutubetrending.netlify.app/</a> &nbsp;&nbsp;&nbsp;&nbsp; <span style="color:#fff">| &nbsp;  &nbsp; It's that </span> <span style="color:lightblue;" >easy</span><span style="color: white">.</span></p>
    ...
    for (var i = 0; i < data.length; ++i){
      page += (
        ...
        `<div class="list">
        <h5><a href="${JSON.parse(data[i]).URL}" target="_blank"><img style="width: 285px; height: 160px; border: 2px solid lightblue; border-radius: 2px;" src="${JSON.parse(data[i]).Thumbnail}"></img></a></h5>
        ...
        <h5><span>"URL"</span>: <a style="color: lightblue;" href="${JSON.parse(data[i]).URL}" target="_blank">"${JSON.parse(data[i]).URL}"</a></h5>
        </div>`
        + '<p class="bracket" style="text-align:left">}</p>'
      )
    }
